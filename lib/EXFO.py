...
class ExfoModule(object):
    ...
    def getODUPMTTITracesExpected(self, strMode):
        command = "LINS" + str(self.shelf) + str(self._slot) + ":SENSe:DATA:TELecom:OTN:ODU4:TTI:%s:EXPected?"%strMode.upper()
        self._session.send(command)
        output = self._session.output
        if "error" in output:
            raise Exception("execute cmd %s on %s failed"%(self, command))
    ...
    def setODUPMTTITracesExpected(self, strMode, strValue):
        command = "LINS" + str(self.shelf) + str(self._slot) + ":SENSe:DATA:TELecom:OTN:ODU4:TTI:%s:EXPected %s"%(strMode.upper(), strValue)
        self._session.send(command)
        output = self._session.output
        if "error" in output:
            raise Exception("execute cmd %s on %s failed"%(self, command))
    ...
    def setODUPMTTITracesStatus(self ,strMode, strStatus):
        command = "LINS" + str(self.shelf) + str(self._slot) + ":SENSe:DATA:TELecom:OTN:ODU4:TTI:TIM %s, %s"%(strMode, strStatus)
        self._session.send(command)
        output = self._session.output
        if "error" in output:
            raise Exception("execute cmd %s on %s failed"%(self, command))
    ...
    def getODUPMTTITracesStatus(self, strMode):
        command = "LINS" + str(self.shelf) + str(self._slot) + ":SENSe:DATA:TELecom:OTN:ODU4:TTI:TIM? %s"%strMode
        self._session.send(command)
        output = self._session.output
        if "error" in output:
            raise Exception("execute cmd %s on %s failed"%(self, command))
