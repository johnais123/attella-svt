...
TC2
    [Documentation]  Test1-----Cx<>Lx----Ly<>Cy-----Test2 /  Test ODU-AIS alarm on line port
    ...              Disable OCH on Lx, Ly will raise ODU-AIS alarm and Test 2 will raise ODU4-AIS alarm
    [Tags]           Sanity  tc2
    Log              Disable local och, remote Line will raise ODU-AIS and Remote Test will raise ODU4-AIS

    Log To Console  Verify Interfaces In Traffic Chain Are Alarm Free
    Verify Interfaces In Traffic Chain Are Alarm Free	

	Log To Console   Disable OUT4 interface on local line port		
	...
	Log To Console   Wait a random time to keep the alarm stable on Attella 	
	${random}=  Evaluate  random.randint(1, 60)  modules=random
	Sleep  ${random}
	
	Log To Console   Verify ODU-AIS was raised on remote line port.		
	...
	Log To Console   Verify ODU4-AIS was raised on remote Test Set.		
	${expectedAlarms_remote_Test_Set}      Set variable      ODU4-AIS   
    Is Alarm Raised  ${testSetHandle2}     ${expectedAlarms_remote_Test_Set} 

	Log To Console   Enable OUT4 interface on local line port		
	...
	Log To Console  Verify Alarms In Traffic Chain Are Alarm Free
	Wait Until Interfaces In Traffic Chain Are Alarm Free
...
TC3
    [Documentation]  Test1-----Cx<>Lx----Ly<>Cy-----Test2 /  Test LOS alarm on line port
    ...              Delete OCH/OTU4/ODU4 on Lx, Ly will raise LOS alarm, and Test 2 will raise OTU4-AIS alarm
    [Tags]           Sanity  tc2
    Log              Delete OCH on Lx, Ly will raise LOS alarm, and Test 2 will raise OTU4-AIS alarm

    Log To Console   Verify Interfaces In Traffic Chain Are Alarm Free
    Verify Interfaces In Traffic Chain Are Alarm Free	

	Log To Console       Delete OCH/OTU4/ODU4 interfaces on local line port		
	...
	Log To Console   Verify LOS was raised on remote line port.		
	@{expectedAlarms_remote_line}      Create List       Loss of Signal  	
	Verify Alarms On Resource            ${odl_sessions}  ${tv['device1__re0__mgt-ip']}  ${remote line odu intf}  ${expectedAlarms_remote_line}
	Verify Interface Operational Status  ${odl_sessions}  ${tv['device1__re0__mgt-ip']}  ${remote line odu intf}  ${OPER_STATUS_OFF}		

	Log To Console   Verify OTU4-AIS was raised on remote Test Set.		
	${expectedAlarms_remote_Test_Set}      Set variable      OTU4-AIS   
    Is Alarm Raised  ${testSetHandle2}     ${expectedAlarms_remote_Test_Set} 

	Log To Console   Re-configure OCH interface on local line port	
    &{local_och_interface}    create_dictionary                 interface-name=${och intf}     description=och-${discription}    interface-type=opticalChannel    
    ...
    @{interface_info}    create list         ${local_och_interface}    
    &{dev_info}          create_dictionary   interface=${interface_info}       
    &{payload}           create_dictionary   org-openroadm-device=${dev_info}
    Send Merge Then Get Request And Verify Output Is Correct    ${odl_sessions}   ${tv['device0__re0__mgt-ip']}  ${payload}	
...
TC4
    [Documentation]  Test1-----Cx<>Lx----Ly<>Cy-----Test2 /  Test OTU4-TTI Mismatch alarm on line port
    ...              Disable OUT4 interface on Lx, Ly will raise OTU4-TTI Mismatch alarm, Lx raise OTU-BDI alarm ，and Test 2 will raise ODU4-AIS alarm
    [Tags]           Sanity  tc2
    Log              OUT4 on Lx send wrong sapi and dapi, Ly will Trail Trace Identifier Mismatch alarm, and Test 2 will raise OTU4-AIS alarm

    Log To Console   Verify Interfaces In Traffic Chain Are Alarm Free
    Verify Interfaces In Traffic Chain Are Alarm Free	

	Log To Console       Delete OCH interface on local line port		
    &{Och_interface}     create dictionary        interface-name=${line och intf}     och=${null}  
    @{interface_info}    create list              ${Och_interface} 
    &{dev_info}          create dictionary        interface=${interface_info}       
    &{payload}           create dictionary        org-openroadm-device=${dev_info}
...
TC4
    [Documentation]  Test1-----Cx<>Lx----Ly<>Cy-----Test2 /  Test OTU4-BDI alarm on line port
    ...              Modify OUT4 SAPI/DAPI on Lx, Ly will raise OTU4-TTI Mismatch alarm, Lx raise OTU-BDI alarm ，and Test 2 will raise ODU4-AIS alarm
    [Tags]           Sanity  tc2
    Log              OUT4 on Lx send wrong sapi and dapi, Ly will Trail Trace Identifier Mismatch alarm, and Test 2 will raise OTU4-AIS alarm
